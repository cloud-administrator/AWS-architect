...
3. Codex GUI は Snowflake ではなく APIM の MCP URL を登録する。
...
Codex GUI に登録するのは Snowflake URL ではなく、APIM が表示する `Server URL` です。
...
7. Redirect URI は Codex GUI に表示される OAuth callback URL を入れます。
...
## 7. Codex GUI に APIM の MCP URL を登録する
...
6. `Name` に `snowflake-apim-private` などを入力します。
7. `URL` に APIM が表示した MCP `Server URL` を入力します。
8. APIM 入口認証が OAuth の場合は、`Authentication` で `OAuth` を選びます。
9. APIM 入口認証が subscription key の場合は、GUI のヘッダー設定に `Ocp-Apim-Subscription-Key` を追加します。
10. `Save` または `Add` を押します。
11. tool 一覧に Snowflake MCP tools が表示されることを確認します。

GUI で固定ヘッダーを入力できず、設定ファイル編集が必要な場合は、Codex GUI から `Open config.toml` を開きます。

```toml
# snowflake-apim-private という MCP server 設定を追加します。
