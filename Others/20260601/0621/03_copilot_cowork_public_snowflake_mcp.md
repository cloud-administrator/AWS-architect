...
この資料の方針は、3資料の中では OAuth の整合性が取りやすい構成です。Microsoft 365 側の OAuth client registration に Snowflake の Client ID / Client Secret を保存し、manifest の `OAuthPluginVault` から参照できるため、Snowflake が Dynamic Client Registration に非対応でも構成しやすいです。

ただし、公開接続なので、Snowflake へどの接続元IPを許可するか、または公開APIM / WAF / Azure Front Doorを前段に置くかを必ず決めてください。ここが未確定のままでは、OAuth が成功しても Snowflake network policy で接続が拒否される可能性があります。

| 項目 | 判定 | 理由 |
|---|---|---|
| `agentConnectors` + `remoteMcpServer` | 動く前提でよい | Microsoft の Copilot Cowork plugin 例と同じ構造です |
| `OAuthPluginVault` | 動く前提でよい | Teams Developer Portal の OAuth registration ID を参照する公式方式です |
| Snowflake OAuth redirect URI | 動く前提でよい | Microsoft 365 plugin OAuth 用 redirect URI を Snowflake に登録する方式です |
| Snowflake public URL への到達 | 要検証 | Microsoft 365 側の接続元を Snowflake network policy で許可できるか確認が必要です |
| 直接Snowflake公開 | 条件付き | 接続元制御が難しい場合は、公開APIM / WAF / Front Door を前段に置く方が安全です |
| tool annotations | 設計上は推奨 | Cowork は MCP `annotations` を参照しますが、Snowflake-managed MCP server の仕様で直接設定できる範囲は実機確認が必要です |
